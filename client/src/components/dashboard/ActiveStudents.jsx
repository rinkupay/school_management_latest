...
const ActiveStudents = () => {
  ...
  const { students, loading, totalStudents, maleStudents, femaleStudents } =
    useSelector((state) => state.activeStudents);
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Fragment>
          <Link to={`/student/${params.id}`} className="action-link-edit">
  ...
  // HANDLE ACTIVE AND INACTIVE STUDENTS
  const handleActiveAndDeactive = (e) => {
    setIsActive(e.target.value);
  };

  useEffect(() => {
    dispatch(fetchActiveStudents({ std, section, isActive }));
  }, [std, section, isActive]);

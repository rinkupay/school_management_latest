...
const StudentUpdate = () => {
  ...
  const [busFeeChecked, setBusFeeChecked] = useState(false);

  const [hostelFee, setHostelFee] = useState("");
  const [hostelFeeChecked, setHostelFeeChecked] = useState(false);
  const [busFeeCheckBox, setBusFeeCheckBox] = useState(
    data?.academicInfo.busFee > 0
  );
  const [hostelFeeCheckBox, setHostelFeeCheckBox] = useState(
  ...
  useEffect(() => {
    ...
    setMonthlyFee(Number(tutionFee) + Number(busFee) + Number(hostelFee));
  ...
  useEffect(() => {
    if (data) {
      ...
      setMonthlyFee(data.academicInfo.monthlyFee || "");
                  ...
                  <form onSubmit={handleRegister}>
                    ...
                    <Box
                      sx={{
                        ...
                        // Optional: If you want to center it within the full viewport height
                        "& button": { m: 1 },
                      }}
                    >
                      <div>
